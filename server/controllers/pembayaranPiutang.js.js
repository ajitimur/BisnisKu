...
class PembayaranPiutangController {
	static async sendInvoice(req, res, next) {
		...
		try {
			...
			console.log(
				"🚀 ~ file: pembayaranPiutang.js.js ~ line 27 ~ PembayaranPiutangController ~ sendInvoice ~ findTransaction",
				findTransaction
			);
			...
			if (findTransaction) {
				...
				const x = new Xendit({
					secretKey:
						"xnd_development_dHe58lZyBgLLnJYRH1O7CTukejaSIBAAkwl5IkKQbw5WBXyzHhVGPcMjdFju0y",
				});
	...
	static async debtPayment(req, res, next) {
		...
		try {
			...
			console.log(
				"🚀 ~ file: pembayaranPiutang.js.js ~ line 92 ~ PembayaranPiutangController ~ debtPayment ~ transaction",
				transaction
			);
